"""向容器内 SQL Server 灌入 E10 表形 + 展厅 seed 数据,并创建只读账号。

由 compose runner 调用;等待 MSSQL 就绪 → 建库 → 建只读账号(d2a_reader,
db_datareader)→ 按 e10_schema.TABLES 建表(_DATE 列用 DATETIME2,

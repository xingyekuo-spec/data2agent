...
def _platform_cfg() -> PlatformConfig:
    return PlatformConfig(templates=str(ROOT / "templates"), landing="landing.sqlite")
...
def test_erp_extract_unknown_when_no_run_in_http_mode(db, pack, tmp_path):
...
def test_push_unknown_when_no_ingest_batch_in_http_mode(db, pack, tmp_path):
    nodes = _nodes(db, pack, _platform_cfg())
...
def test_mcp_failed_when_down(db, pack, tmp_path):
    nodes = _nodes(db, pack, _platform_cfg(), probes={"mcp": lambda: (False, "tcp")})
    assert nodes["mcp"]["status"] == "failed"


def test_mcp_healthy_and_stale_follows_objects(db, pack, tmp_path):
    _mk_obj(db, pack, "Customer", 5, FRESH)
    nodes = _nodes(db, pack, _platform_cfg(), probes={"mcp": lambda: (True, "http")})
    assert nodes["mcp"]["status"] == "healthy"

    _mk_raw(db, "CUSTOMER", 6, obs.aware("2026-07-18T11:59:00").isoformat())
    nodes2 = _nodes(db, pack, _platform_cfg(), probes={"mcp": lambda: (True, "http")})
...
def test_pipeline_fixed_seven_nodes_and_overall(db, pack, tmp_path):
    result = obs.build_pipeline(db, pack, _platform_cfg(), SOURCE, now=NOW)
    ...
    result2 = obs.build_pipeline(db, pack, _platform_cfg(), SOURCE, now=NOW)
...
def test_sqlite_failure_is_unknown_not_idle(db, pack, tmp_path):
    ...
    # push(http sink, 无批次)与 mcp(无探测)受影响
    assert nodes["push"]["status"] == "unknown"
    assert nodes["mcp"]["status"] == "unknown"
...
def test_run_facts_filter_by_source(db, pack, tmp_path):
    ...
    nodes_b = {n["node"]: n for n in obs.compute_nodes(
        db, pack, _platform_cfg(), "source_b", now=NOW)}
...
def test_last_success_and_failure_are_independent(db, pack, tmp_path):
    ...
    erp2 = _nodes(db2, pack, _platform_cfg())["erp"]
...
def test_orphaned_running_does_not_override_newer_success(db, pack, tmp_path):
    ...
    erp = _nodes(db, pack, _platform_cfg())["erp"]
...
def test_paused_is_warning_not_failure(db, pack, tmp_path):
    ...
    erp2 = _nodes(db, pack, _platform_cfg())["erp"]
...
def test_objects_healthy_with_default_threshold(db, pack, tmp_path):
    """PlatformConfig 默认 30m 节奏,raw/objects 可判健康和新鲜度。"""
    _mk_raw(db, "CUSTOMER", 5, FRESH)
    _mk_obj(db, pack, "Customer", 5, FRESH)

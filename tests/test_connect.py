...
def test_full_sync_lands_everything(source_db, landing):
    report = full_sync(_adapter(source_db, landing=landing), landing, SOURCE)
...
def test_sync_idempotent(source_db, landing):
    full_sync(_adapter(source_db, landing=landing), landing, SOURCE)
    before = landing.count(SOURCE, "SALES_ORDER")
    full_sync(_adapter(source_db, landing=landing), landing, SOURCE)
    assert landing.count(SOURCE, "SALES_ORDER") == before, "重跑必须幂等"


def test_upsert_reflects_source_change(source_db, landing):
    full_sync(_adapter(source_db, landing=landing), landing, SOURCE)

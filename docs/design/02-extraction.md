...
| 门槛 | 目标机制 | 当前状态 |
| --- | --- | --- |
| 跨机加密传输 | 平台反向代理终止 TLS 或等价方案;中间 URL 强制 `https://` | 代码默认 fail-closed；现场证书/反代验收待完成 |
| 端到端提交 | 批次 ID + 内容摘要 + 持久回执 + generation 屏障 | 已实现；现场断网/回包丢失演练待完成 |
| 跨机对账 | 中间算源侧统计,平台比对并返回差异段,重抽 + 主键 diff 软删 | E6b 已实现（含 deep 调度） |
| raw 数据保护 | 最小目录权限、数据盘/备份加密、保留与清理策略、敏感列裁剪决策 | 在线一致备份/保留任务已实现；目录与介质策略现场确认 |
| 凭据治理 | ingest/console/MCP 凭据分离、轮换、吊销、主体审计 | 待建 |
| SQLite 适用性 | 容量/并发/延迟/checkpoint/备份恢复压测与换库阈值 | 请求热路径无 DDL、在线备份已实现；压测/恢复演练待验证 |
...
| E6b | 部署(§12):对账劈开(中间源侧统计 ↔ 平台落地侧比对) | ✅ 跨机 L1/L2、staging 键 diff 与软删集成测试通过 |
...
### 12.3 拆机可靠性能力(E6a、ingest v3 提交协议与 E6b 已实现)
...
ingest v3 在平台事务内同时提交 raw upsert 与不可变批次 receipt；中间机校验回执中的
batch ID 和内容摘要后才推进对应批次水位。一次 sync 另有 generation 清单与表完成集合，
只有完整 generation 可被平台 apply worker 领取。

**v0.4 · 端到端批次提交协议(已实现)**:

- 请求携带不可重复 batch ID、表运行 ID、generation ID、结构、行数和内容摘要;
- 平台事务提交后持久化批次 receipt;
...
中间重抽这些段流式推送(行 + 当前主键全集)→ 平台将键写入 staging 表并用 SQL
反连接打软删，避免任一端把全表主键集合装入内存。

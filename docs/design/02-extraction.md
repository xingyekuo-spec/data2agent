...
> 状态:详设 v0.2(2026-07-11 补现场部署拓扑 §12)· 实现目录:`data2agent/connect/`(依赖组 `connect`:pyodbc / apscheduler / psycopg)· 抽取框架 E1–E5 已实现;部署模式 A 的改动(§12.3)待建
...
- 调度编排 UI —— 调度只经 connect.yaml + CLI 配置;运行监控与复核由
  运维控制台提供(docs 05;2026-07-11 边界调整,原划商业版)。
...
| E6 | 部署模式 A(§12):connect 推送 sink + 对账劈开 | 中间/平台两机(或双进程)集成:中间推送、平台落地、对账跨机比对 |
...
### 12.1 采用模式 A:内网流式 agent + 出站推送(数据平台标准姿势)

这是数据平台产品接入 on-prem 源的**主流标准做法**(Azure 自托管集成运行时 SHIR、
...
**为什么选 A(安全依据)**:数据平台是链路里唯一有外网的机器,即唯一对外暴露点。
A 把"读 ERP + ERP 凭据"留在**不上外网**的中间服务器 —— 平台即便从外网被打穿,
...
### 12.3 落地模式 A 需要的改动(待建,诚实声明)

现有 connect 是"落地再映射"引擎(适配器+增量+对账+落地全在一处),
**直接满足同机部署与备选模式 B**。落地模式 A 需两项新增,其余全部复用:

| 改动 | 说明 | 状态 |
| --- | --- | --- |
| connect **推送 sink 模式** | 抽取的 raw 批次不写本地落地库,改 POST 到数据平台接收端;适配器 / 水位 / 增量逻辑复用,只换出口 | 待建 |
| **对账劈开** | 源侧统计在中间跑、落地侧在平台跑,比对后由平台侧打软删 | 待建 |

复用不变:游标增量 + 回看 + 软删 + 隔离 + 熔断 + 逐条审计 + 错峰窗口。

### 12.4 备选模式 B(若中间必须是纯哑转发)

若 IT 坚持中间**连无状态流式 agent 都不能跑**、只能做 L4 转发:退回 **pull** ——
中间用 nginx stream 做 TCP 隧道,connect **整体跑在数据平台**、DSN 指向隧道读 ERP
(零改动、复用现成 connect、对账统一在平台)。代价:ERP 只读凭据 + 一条隧道 SQL
通路落在**有外网的平台**上,靠只读账号 + 平台外网出站白名单 + 审计压住。
**A 与 B 的取舍由 IT 威胁模型定** —— 决策问题见附录核对项 ⑦。

### 12.5 只读约束下的同步机制(与 A/B 无关)

ERP 只授只读 → **CDC / 日志捕获被封死**(SQL Server CDC / Change Tracking 需提权开启,
只读账号做不到)。故无论 A/B,增量都退回**游标(水位)增量**,其删除 / 静默改盲区
...
部署拓扑(§12)与 IT 定的决策项:⑧ 中间服务器能否跑无状态流式 agent(能→模式 A;只能哑转发→模式 B);⑨ 中间→数据平台的出站是否放行(模式 A 需要);⑩ 数据平台的外网出站是否可收窄为白名单(仅 LLM 端点等);⑪ ERP 是否可能开只读之外的 CDC/Change Tracking 权限(可则重估 CDC,否则游标增量 + 对账)。

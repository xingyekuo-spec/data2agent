...
> 状态:详设 v0.2(2026-07-11 补现场部署拓扑 §12)· 实现目录:`data2agent/connect/`(依赖组 `connect`:pyodbc / apscheduler / psycopg)· 抽取框架 E1–E5 已实现;部署改动(§12.3)待建
...
| E6 | 部署(§12):connect 推送 sink + 对账劈开 | 中间/平台两机(或双进程)集成:中间推送、平台落地、对账跨机比对 |
...
本拓扑把抽取放在中间、落地放在平台,需两项新增,其余全部复用:

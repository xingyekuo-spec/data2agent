...
| `propose_action` | —— | 「说」档建议卡:结论 + 依据(须引用 `meta.query_id + meta.result_digest`)→ 卡片 |

行为契约(违反即 bug,均有测试锁定):

1. **业务数据只读**:published 数据查询以 `mode=ro` 打开；SQL 只由 binding 生成(`data2agent/mapping.py`)，表和字段天然白名单。M5 仅在独立短事务写入 evidence 元数据与审计，不写业务数据;
...
"说"档的每次可引用查询均返回持久的 `meta.query_id`、`meta.result_digest` 与有界结果摘要；建议卡的每条依据必须同时引用 query ID 和摘要，且仅能使用同一主体、同一会话内尚未到期的 evidence。卡片原子固化被引用查询的版本、摘要和口径警示
(draft binding / draft 指标 / caveats)，并附治理声明。它不自动校验自然语言 claim 中每个数字是否等于查询结果。部署以 `max_tier`
设置档位上限(默认“说”),超档动作被硬拒绝并说明治理边界。

### 3.1 v0.3 查询证据契约(已实现)

v0.3 已将进程级简单序号升级为持久、隔离、可审计的证据对象。单 Token 部署映射为唯一部署主体；请求体和 MCP 工具参数不能伪造主体或会话。
...
- evidence 只能引用同一 `principal`、同一 `session_id` 且仍有效的 query；M5 不提供跨会话复用旁路;
- 引用不同 `dataset_version` 的证据一律拒绝;
...
| 主体不一致 | `403`,不透露目标证据内容 |
| 会话不一致 | `409 evidence_session_mismatch` |
| 数据集版本不一致 | `409 dataset_version_mismatch` |

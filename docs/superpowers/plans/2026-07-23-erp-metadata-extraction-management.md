...
> 状态：M0–M7 已完成（展厅/Mock 已迁出产品包；见 2026-07-24 迁移核对清单）
> 制定日期：2026-07-23
> 最近修订：2026-07-24
> 目标版本：v0.5.0（已对齐仓库版本号）
> 前置设计：[中间机独立抽取表配置](../../design/06-middle-table-extraction-config.md)
> 本文为已完成的实施方案存档；功能以代码与路线图为准。
...
仍保留不可关闭的协议版本校验（以支持列表为准，而非应用版本成对一致）:

1. 平台健康接口返回 `active_ingest_protocol_version` 与

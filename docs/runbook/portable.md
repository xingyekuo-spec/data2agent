...
**必须传输 zip 原文件，到 Windows 后再解压**；禁止「在 Mac 上解压后把文件夹拷进 U 盘」。
macOS 向 FAT32/exFAT U 盘写入时会生成 `._*` AppleDouble 伴随文件，这些二进制文件
混入 `app\templates` 会被当作 YAML 模板读取，导致控制台启动报 `UnicodeDecodeError`
（且首配模式不加载模板，第一次启动不会暴露，首配完成后的第二次启动才崩溃）。
推荐解压工具：Windows 资源管理器或 7-Zip；避免使用会对文本文件「智能转码」的
国产压缩软件。

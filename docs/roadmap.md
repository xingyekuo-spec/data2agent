...
> 状态: `v0.3.0` 之后完成 v0.5 ERP 元数据 M0–M7（产品包无 showroom，Console 仅真实 API）;更新于 2026-07-24。

data2agent 已完成 v0.2 可观察控制台和 v0.3 可验证数据链,以及中间机元数据发现与抽取表管理
（见 [ERP 元数据实施计划](superpowers/plans/2026-07-23-erp-metadata-extraction-management.md)）。
...
进行中：无（M7 已完成：产品包无 showroom，Console 仅真实 API）。
